...
func calculateTripVotes (votes:Array<ClassTripDestination>)->String
{
  ...
  return "The class votes for beach : \(beachVotes) , chocolate factory \(chocolateVotes) "
}
let results = calculateTripVotes(votes: tripDestinationVotes)


//Exercise2
func calculateMacotVotes (votes:Array<SchoolMascotOption>)->String{
  ...
  return "result the votes : salmander : \(salamanderCount) , marmot \(marmotCount),neither\(neitherCount)"
}
let mascotResult = calculateMacotVotes(votes: mascotVotes)
